...
export function createGameOverScreen(container, { score, bestScore, isNewBest, onRestart, onHome }) {
  ...
  overlay.querySelector('#btn-home').addEventListener('click', () => {
    if (onHome) onHome();
    else if(window.parent !== window) { window.parent.postMessage('close-game', '*'); } else { (function(){ if(window.parent !== window) window.parent.postMessage('close-game', '*'); else window.location.href = import.meta.env.BASE_URL; })() }
  });
